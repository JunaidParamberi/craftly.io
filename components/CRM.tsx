...
import { 
  ...
  Sparkles, Loader2, MessageSquare, CheckSquare, Square,
  Upload, RefreshCw, Download, Check, Users,
  Wand2, History, Archive, RotateCcw,
  FileImage, Palette,
  Sliders,
...
import { Input, Button, Heading, Card, Badge, Select, Label, EmptyState, Skeleton, SkeletonList, LoadingProgress } from './ui/Primitives';
...
const CRM: React.FC = () => {
  ...
  // Only count 'Invoice' type to avoid doubling with LPOs
  const getClientLTV = (clientName: string) => {
    return invoices
      .filter(inv => inv.clientId === clientName && inv.type === 'Invoice' && inv.status === 'Paid')
  ...
  const handleDownload = () => {
    ...
    link.download = `CREAFTLYAI_CREATIVE_${Date.now()}.jpg`;
  ...
  const generateAiCampaign = async () => {
    ...
    try {
      ...
      const systemInstruction = `You are CreaftlyAI. UAE freelancer marketing expert. Goal: "${campaignData.subject}". Generate punchy, premium copy. Return JSON {subject, body}.`;
  ...
  if (loading) {
    return (
      <div className="space-y-4 lg:space-y-6 animate-enter pb-12">
        <header className="flex flex-col md:flex-row md:items-start justify-between gap-4 px-1 shrink-0">
          <div className="space-y-4">
            ...
            <Skeleton variant="rounded" width="300px" height={40} className="h-10 rounded-2xl" />
          </div>
          <div className="flex items-center gap-4">
            <Skeleton variant="rounded" width={200} height={48} className="h-12 rounded-2xl" />
            <Skeleton variant="rounded" width={150} height={48} className="h-12 rounded-2xl" />
          </div>
        </header>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <Card key={i} className="p-8 space-y-4" hover={false}>
    ...
    <div className="space-y-4 lg:space-y-6 animate-enter pb-12 h-full overflow-y-auto no-scrollbar">
      ...
      <header className="flex flex-col md:flex-row md:items-start justify-between gap-4 px-1 shrink-0">
        <div>
          ...
          <div className="flex items-center p-1 bg-[var(--bg-card-muted)] border border-[var(--border-ui)] rounded-2xl w-fit mt-8 shadow-2xl">
            <button 
              onClick={() => setActiveTab('list')} 
              className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all ${activeTab === 'list' ? 'bg-[var(--accent)] text-white shadow-lg shadow-indigo-500/20' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}
            >
              Registry
            </button>
            {hasCampaignPermission && (
              <button 
                ...
                className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all ${activeTab === 'campaigns' ? 'bg-[var(--accent)] text-white shadow-lg shadow-indigo-500/20' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}
              >
                Campaign Engine
              </button>
            )}
        ...
        {activeTab === 'list' && (
          <div className="flex flex-wrap items-center gap-4 mt-12 md:mt-0">
            <div className="relative group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--text-secondary)] group-focus-within:text-[var(--accent)] transition-colors pointer-events-none" size={16} />
              <input 
                type="text" 
                placeholder="Search targets..." 
                className="w-full sm:w-72 h-12 pl-12 pr-4 bg-[var(--input-bg)] border border-[var(--border-ui)] rounded-[1.25rem] text-xs font-bold outline-none focus:border-[var(--accent)] transition-all shadow-sm text-[var(--text-primary)] touch-manipulation" 
                value={searchQuery} 
      ...
      {activeTab === 'list' ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4 max-w-6xl">
          {filteredClients.map(client => {
            ...
            return (
              <Card 
                key={client.id} 
                className="p-8 relative group border-[var(--border-ui)] bg-[var(--bg-card)] hover:border-[var(--accent)]/40 transition-all rounded-[2.5rem] shadow-2xl overflow-hidden min-h-[180px] cursor-pointer"
              ...
              >
                 <div className="absolute top-6 right-6 flex gap-2 opacity-0 group-hover:opacity-100 transition-all z-10" onClick={e => e.stopPropagation()}>
                  <button onClick={() => { setEditingId(client); setFormData(client); setShowForm(true); }} className="p-2.5 bg-[var(--bg-card-muted)] border border-[var(--border-ui)] rounded-xl text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-all shadow-xl"><Edit2 size={14}/></button>
                  <button onClick={() => setConfirmDeleteId(client.id)} className="p-2.5 bg-[var(--bg-card-muted)] border border-[var(--border-ui)] rounded-xl text-rose-500 hover:bg-rose-600 hover:text-white transition-all shadow-xl"><Trash2 size={14}/></button>
                </div>
                
                <div className="flex items-center gap-6 mb-8">
                  <div className="w-16 h-16 rounded-[1.25rem] bg-[var(--accent)] text-white flex items-center justify-center font-black text-2xl shadow-xl shrink-0 uppercase border border-white/10">{client.name.charAt(0)}</div>
                  <div className="min-w-0 flex-1">
                    <h3 className="text-xl font-black text-[var(--text-primary)] truncate uppercase tracking-tight leading-none mb-3">{client.name}</h3>
                    <div className="flex items-center gap-2 opacity-60">
                      <UserCheck size={12} className="text-[var(--accent)]" />
                      <span className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest truncate">{client.contactPerson}</span>
                    </div>
                  </div>
                </div>
                
                <div className="h-px bg-[var(--border-ui)] w-full mb-8" />
                ...
                <div className="flex justify-between items-end relative">
                   ...
                   <div className="text-right">
                      <p className="text-[8px] font-black text-[var(--text-secondary)] uppercase tracking-[0.3em] mb-1">Total Worth</p>
                      <div className="flex items-baseline justify-end gap-1.5">
                         <span className="text-[10px] font-black text-[var(--text-secondary)] uppercase">AED</span>
                         <p className="text-3xl font-black text-[var(--text-primary)] tabular-nums tracking-tighter leading-none">{ltvValue.toLocaleString()}</p>
                      </div>
                   </div>
                </div>
              </Card>
        ...
        <div className="flex flex-col lg:flex-row bg-[var(--bg-card)] border border-[var(--border-ui)] rounded-[2.5rem] overflow-hidden min-h-[700px] lg:min-h-[750px] shadow-2xl relative">
          <aside className="w-full lg:w-[380px] bg-[var(--bg-card-muted)] lg:border-r border-[var(--border-ui)] p-6 lg:p-8 flex flex-col shrink-0 overflow-y-auto custom-scroll">
            <header className="mb-8 flex justify-between items-start shrink-0">
               <div className="space-y-2">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-xl ${campaignChannel === 'WHATSAPP' ? 'bg-emerald-600' : 'bg-[var(--accent)]'}`}>
                    {campaignChannel === 'WHATSAPP' ? <MessageSquare size={24} /> : <Mail size={24} />}
                  </div>
                  <h3 className="text-xl font-black uppercase tracking-tighter text-[var(--text-primary)] leading-none">Studio</h3>
               </div>
               <div className="flex gap-2">
                  <button onClick={() => { setCampaignAsset(null); setBrandedAsset(null); setCampaignData({subject:'', body:'', targetStatus:'ALL'}); setVisualPrompt(''); }} className="p-2.5 bg-[var(--bg-card)] hover:bg-[var(--accent)]/10 text-[var(--accent)] border border-[var(--border-ui)] rounded-xl transition-all group" title="Reset Node"><RotateCcw size={18} className="group-hover:-rotate-90 transition-transform" /></button>
                  <button onClick={() => setCampaignViewMode(campaignViewMode === 'NEW' ? 'ARCHIVE' : 'NEW')} className={`p-2.5 border rounded-xl transition-all ${campaignViewMode === 'ARCHIVE' ? 'bg-[var(--accent)] text-white border-[var(--accent)]' : 'bg-[var(--bg-card)] text-[var(--text-secondary)] border-[var(--border-ui)] hover:text-[var(--text-primary)]'}`} title="Historical Archive"><History size={18}/></button>
               </div>
            </header>
            ...
            {campaignViewMode === 'ARCHIVE' ? (
              <div className="flex-1 space-y-3 animate-enter pb-6">
                 {campaigns.length > 0 ? campaigns.map(camp => (
                   <button key={camp.id} onClick={() => recallCampaign(camp)} className="w-full p-4 bg-[var(--bg-card)] border border-[var(--border-ui)] rounded-2xl text-left group hover:border-[var(--accent)] transition-all active:scale-[0.98] overflow-hidden relative shadow-md">
                      ...
                      <div className="relative z-10">
                        <div className="flex justify-between items-start mb-2">
                           <div className={`p-1 rounded-md ${camp.channel === 'WHATSAPP' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-[var(--accent)]/10 text-[var(--accent)]'}`}>{camp.channel === 'WHATSAPP' ? <MessageSquare size={12}/> : <Mail size={12}/>}</div>
                           <span className="text-[8px] font-black text-[var(--text-secondary)] uppercase tracking-widest tabular-nums">{new Date(camp.timestamp).toLocaleDateString()}</span>
                        </div>
                        <h4 className="text-[10px] font-black uppercase text-[var(--text-primary)] truncate mb-1 pr-12">{camp.subject || '(NO SUBJECT)'}</h4>
                        <p className="text-[9px] text-[var(--text-secondary)] line-clamp-2 italic pr-8 leading-relaxed">"{camp.body}"</p>
                      </div>
                   </button>
              ...
              <div className="space-y-6 flex-1 animate-enter pb-6">
                <div className="flex p-1 bg-[var(--input-bg)] border border-[var(--border-ui)] rounded-xl shadow-inner">
                   <button onClick={() => setCampaignChannel('EMAIL')} className={`flex-1 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${campaignChannel === 'EMAIL' ? 'bg-[var(--accent)] text-white shadow-md' : 'text-[var(--text-secondary)]'}`}>Email</button>
                   <button onClick={() => setCampaignChannel('WHATSAPP')} className={`flex-1 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${campaignChannel === 'WHATSAPP' ? 'bg-emerald-600 text-white shadow-md' : 'text-[var(--text-secondary)]'}`}>WhatsApp</button>
                </div>

                <Input label="Objective" placeholder="What is the goal?" value={campaignData.subject} onChange={e => setCampaignData({...campaignData, subject: e.target.value})} className="font-black !h-12" />
                
                <div className="space-y-2">
                   <div className="flex justify-between items-center px-1">
                      <Label className="uppercase tracking-widest">Draft Manifesto</Label>
                      <button onClick={generateAiCampaign} disabled={isAiGenerating} className="text-[8px] font-black uppercase text-[var(--accent)] flex items-center gap-1.5 hover:underline disabled:opacity-20">{isAiGenerating ? <Loader2 size={10} className="animate-spin"/> : <Sparkles size={10}/>} AI Copy</button>
                   </div>
                   <textarea className="w-full h-24 lg:h-32 bg-[var(--input-bg)] border border-[var(--border-ui)] rounded-2xl p-4 text-sm font-semibold outline-none focus:border-[var(--accent)] transition-all shadow-inner leading-relaxed resize-none text-[var(--text-primary)] placeholder:text-[var(--text-secondary)]" placeholder="Draft copy..." value={campaignData.body} onChange={e => setCampaignData({...campaignData, body: e.target.value})} />
                </div>

                <div className="pt-4 space-y-6 border-t border-[var(--border-ui)]">
                   <div className="flex justify-between items-center px-1">
                      <Label className="uppercase tracking-widest">Visual Style</Label>
                   ...
                   <div className="grid grid-cols-2 gap-4">
                      <Select label="Ratio" value={aspectRatio} onChange={e => setAspectRatio(e.target.value)} className="!h-10 !text-[10px]">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <button onClick={() => attachedImage ? setAttachedImage(null) : fileInputRef.current?.click()} className={`w-full h-10 px-3 rounded-lg border flex items-center justify-center gap-2 transition-all ${attachedImage ? 'bg-[var(--accent)] text-white border-[var(--accent)]' : 'bg-[var(--input-bg)] border-[var(--border-ui)] text-[var(--text-secondary)] hover:border-[var(--accent)]'}`}>
            ...
            <div className="mt-auto pt-6 border-t border-[var(--border-ui)] shrink-0">
               <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[8px] font-black text-[var(--text-secondary)] uppercase tracking-widest mb-0.5">Target Cluster</p>
                    <p className="text-2xl font-black text-[var(--text-primary)] tabular-nums">{selectedRecipientIds.length}</p>
                  </div>
                  <button onClick={() => { if(selectedRecipientIds.length) { setCurrentIndex(0); executeCurrentDispatch(); } }} disabled={selectedRecipientIds.length === 0} className={`h-12 px-6 rounded-xl shadow-lg uppercase tracking-[0.2em] font-black text-[10px] ${campaignChannel === 'WHATSAPP' ? 'bg-emerald-600' : 'bg-[var(--accent)]'} text-white active:scale-95 transition-all disabled:opacity-20`}>Execute Protocol</button>
               </div>
            </div>
          </aside>

          <main className="flex-1 bg-[var(--bg-card)] p-6 lg:p-8 overflow-y-auto custom-scroll flex flex-col relative">
             ...
             <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 relative z-10">
                <div>
                   <h4 className="text-base lg:text-lg font-black uppercase tracking-[0.4em] text-[var(--text-primary)]">Visual Compositor</h4>
                   <p className="text-[9px] font-black text-[var(--accent)] uppercase tracking-widest mt-1 opacity-80">Synthesis Hub</p>
                </div>
                <div className="flex gap-3">
                   <button onClick={generateVisualAsset} disabled={isGeneratingPoster} className="px-5 h-11 bg-[var(--accent)] text-white rounded-xl font-black uppercase text-[10px] tracking-widest shadow-xl flex items-center justify-center gap-2 hover:scale-105 active:scale-95 transition-all">
             ...
             <div className="flex-1 flex flex-col lg:flex-row gap-6 relative z-10 min-h-0 mb-8">
                ...
                <div className="flex-1 flex flex-col min-h-0 relative group">
                  <div className="flex-1 bg-[var(--bg-card-muted)] border border-[var(--border-ui)] rounded-[2.5rem] shadow-2xl flex items-center justify-center overflow-hidden relative group backdrop-blur-3xl min-h-[400px]">
                     {isGeneratingPoster ? (
                ...
                {campaignAsset && (
                  <div className="w-full lg:w-[320px] space-y-6 bg-[var(--bg-card-muted)] p-6 rounded-[2.5rem] border border-[var(--border-ui)] animate-enter shrink-0 backdrop-blur-3xl shadow-xl h-fit">
                    <header className="flex items-center gap-3 text-[var(--accent)] border-b border-[var(--border-ui)] pb-4">
                       <Palette size={18} />
                    ...
                    <div className="space-y-6">
                       <div className="space-y-4">
                          <div className="flex items-center justify-between p-3 bg-[var(--bg-card)] rounded-xl border border-[var(--border-ui)]">
                             <span className="text-[9px] font-black uppercase tracking-widest text-[var(--text-secondary)]">Branding</span>
                             <input type="checkbox" checked={designSettings.showLogo} onChange={e => setDesignSettings({...designSettings, showLogo: e.target.checked})} className="w-5 h-5 rounded-md bg-[var(--input-bg)] border-[var(--border-ui)] text-[var(--accent)]" />
                          </div>
                          <div className="relative">
                             <input ref={logoUploadRef} type="file" className="hidden" accept="image/*" onChange={handleLogoUpload} />
                             <button onClick={() => logoUploadRef.current?.click()} className="w-full h-11 bg-[var(--accent)]/10 text-[var(--accent)] border border-[var(--accent)]/20 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-2 hover:bg-[var(--accent)]/20 transition-all">
                          ...
                          <Select label="Anchor" value={designSettings.logoPosition} onChange={e => setDesignSettings({...designSettings, logoPosition: e.target.value as any})} disabled={!designSettings.showLogo} className="!h-10 !text-[10px] font-bold">
                       ...
                       <div className="h-px bg-[var(--border-ui)]" />

                       <div className="space-y-4">
                          <textarea className="w-full bg-[var(--input-bg)] border border-[var(--border-ui)] rounded-xl p-4 text-[11px] font-black uppercase tracking-tight text-[var(--text-primary)] outline-none focus:border-[var(--accent)] h-20 shadow-inner resize-none" value={designSettings.overlayText} onChange={e => setDesignSettings({...designSettings, overlayText: e.target.value})} placeholder="OVERLAY TEXT..." />
                          <div className="space-y-3">
                             <Label className="!text-[8px] uppercase tracking-widest">Filter Synthesis</Label>
                             <div className="grid grid-cols-5 gap-2">
                                {CAMPAIGN_PATTERNS.map(p => (
                                  <button key={p.id} onClick={() => setDesignSettings({...designSettings, pattern: p.id})} className={`h-8 rounded-lg border-2 transition-all relative ${designSettings.pattern === p.id ? 'border-[var(--accent)] scale-110 shadow-lg' : 'border-[var(--border-ui)] hover:border-[var(--accent)]/30'}`} style={{ backgroundColor: p.color === 'transparent' ? 'var(--bg-card)' : p.color }}>
                                    {designSettings.pattern === p.id && <div className="absolute inset-0 flex items-center justify-center bg-[var(--accent)]/20"><Check size={12} className="text-white"/></div>}
                                  </button>
                                ))}
                             </div>
                          </div>
                          <div className="flex items-center gap-3 p-3 bg-[var(--bg-card)] rounded-xl border border-[var(--border-ui)]">
                             <Sliders size={14} className="text-[var(--text-secondary)] shrink-0" />
                             <div className="flex-1 space-y-1">
                               <input type="range" min="10" max="100" value={designSettings.opacity} onChange={e => setDesignSettings({...designSettings, opacity: parseInt(e.target.value) || 100})} className="w-full accent-[var(--accent)] h-1 bg-[var(--input-bg)] rounded-full" />
                               <div className="flex justify-between text-[7px] font-black text-[var(--text-secondary)] uppercase tracking-widest"><span>Opacity</span><span className="text-[var(--accent)]">{designSettings.opacity}%</span></div>
                             </div>
                          </div>
                       </div>
                    </div>
             ...
             <div className="mt-auto bg-[var(--bg-card-muted)] p-6 lg:p-8 rounded-[2.5rem] border border-[var(--border-ui)] shrink-0 shadow-2xl relative overflow-hidden">
                <div className="absolute top-0 right-0 p-8 opacity-[0.01] pointer-events-none"><Target size={120}/></div>
                <div className="flex items-center justify-between mb-6 border-b border-[var(--border-ui)] pb-4 relative z-10">
                   <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-[var(--accent)]/10 text-[var(--accent)] rounded-xl flex items-center justify-center border border-[var(--accent)]/20 shadow-lg"><Target size={18}/></div>
                      <div>
                         <h5 className="text-sm font-black uppercase tracking-[0.4em] text-[var(--text-primary)] leading-none">Target Registry</h5>
                         <p className="text-[8px] font-black text-[var(--text-secondary)] uppercase tracking-[0.3em] mt-1">Map recipient nodes</p>
                      </div>
                   </div>
                   <button onClick={() => setSelectedRecipientIds(eligibleRecipients.map(c => c.id))} className="px-4 py-2 bg-[var(--bg-card)] hover:bg-[var(--accent)] rounded-lg text-[8px] font-black uppercase tracking-widest text-[var(--accent)] hover:text-white transition-all border border-[var(--border-ui)]">Map All Filtered</button>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 max-h-[220px] overflow-y-auto no-scrollbar pr-2 relative z-10">
                   {eligibleRecipients.map(c => (
                      <button 
                        ...
                        className={`flex items-center gap-3 p-4 rounded-2xl border-2 transition-all text-left group shrink-0 ${selectedRecipientIds.includes(c.id) ? 'bg-[var(--accent)]/10 border-[var(--accent)] shadow-md' : 'bg-[var(--bg-card)] border-transparent hover:border-[var(--border-ui)] shadow-sm'}`}
                      >
                         <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-lg transition-transform group-active:scale-90 ${selectedRecipientIds.includes(c.id) ? 'bg-[var(--accent)] text-white' : 'bg-[var(--bg-card-muted)] text-[var(--text-secondary)]'}`}>
                            {selectedRecipientIds.includes(c.id) ? <CheckSquare size={20}/> : <Square size={20}/>}
                         </div>
                         <div className="min-w-0 flex-1">
                            <p className="text-[11px] font-black text-[var(--text-primary)] truncate uppercase tracking-tight leading-none mb-1.5">{c.name}</p>
        ...
        <div className="exec-modal-overlay">
          <div className="exec-modal-container max-w-4xl animate-pop-in border-none shadow-[0_0_100px_rgba(0,0,0,0.5)] bg-[var(--bg-card)]">
            <header className="p-8 border-b border-[var(--border-ui)] flex justify-between items-center bg-[var(--bg-card)]">
              ...
              <button onClick={() => setShowForm(false)} className="p-3 text-[var(--text-secondary)] hover:text-rose-500 transition-all hover:bg-[var(--bg-card-muted)] rounded-2xl"><X size={28} /></button>
            </header>
