...
import { 
  ...
  Calculator, Search, FileSignature, Clock, Target, ListChecks, Briefcase
} from 'lucide-react';
  ...
  return (
    <div className="relative space-y-2" ref={containerRef}>
      ...
      <button type="button" onClick={() => setIsOpen(!isOpen)}
        className="w-full h-11 px-4 bg-[var(--input-bg)] border border-[var(--border-ui)] rounded-xl flex items-center justify-between text-left hover:border-[var(--accent)] transition-all">
      ...
      {isOpen && (
        <div className="absolute z-[10001] top-full left-0 w-full mt-2 bg-[var(--bg-card)] border border-[var(--border-ui)] rounded-xl shadow-xl overflow-hidden max-h-64 overflow-y-auto custom-scroll animate-pop-in">
          {onAction && <button type="button" onClick={() => { onAction(); setIsOpen(false); }} className="w-full px-5 py-3 text-left text-[10px] font-black uppercase tracking-widest text-[var(--accent)] border-b border-[var(--border-ui)] hover:bg-[var(--accent)]/5 flex items-center gap-3"><UserPlus size={14} /> {actionLabel || 'Add New'}</button>}
          {options.map((opt: any) => (
            <button key={opt.id} type="button" onClick={() => { onChange(opt.id); setIsOpen(false); }} className={`w-full px-5 py-3 text-left text-[10px] font-bold uppercase tracking-widest hover:bg-[var(--accent)]/5 transition-all flex items-center justify-between ${value === opt.id ? 'text-[var(--accent)]' : 'text-[var(--text-secondary)]'}`}>
  ...
  const handleAiSmartSetup = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Project idea: "${aiPrompt}". Suggest: title, description, days, and budget. Use natural sentence case for titles and text.`,
        config: { 
  ...
  return (
    <div className="space-y-8 animate-enter pb-20">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <Heading sub={`Viewing ${proposals.length} active nodes`}>Missions</Heading>
        <div className="flex gap-3">
          ...
          />
          <Button onClick={() => navigate('/projects/new')} icon={Plus}>Start Mission</Button>
        </div>
      </header>
      ...
      {proposals.length > 0 ? (
            ...
            return (
              <Card key={prop.id} className="group flex flex-col relative overflow-hidden">
                ...
                <div className="mt-auto pt-6 border-t border-[var(--border-ui)] flex justify-between items-end">
                   <div>
                      <p className="text-[9px] font-black uppercase text-slate-500 tracking-widest mb-1 opacity-50">Worth</p>
                      <p className="text-2xl font-bold tabular-nums tracking-tighter text-[var(--text-primary)]">AED {prop.budget.toLocaleString()}</p>
                   </div>
                   <Button onClick={() => navigate(`/projects/${prop.id}`)} variant="primary" size="icon" icon={Eye} />
                </div>
              </Card>
            );
          })}
        ...
        <EmptyState 
          icon={Briefcase} 
          title="The workspace is quiet." 
          description="Ready to launch your next strategic mission? Every legacy begins with an initialization node."
          action={<Button onClick={() => navigate('/projects/new')} icon={Plus}>Initialize Mission</Button>}
        />
      )}
        ...
        <div className="exec-modal-overlay">
          <div className="exec-modal-container !max-w-4xl animate-pop-in">
             <header className="p-6 border-b border-[var(--border-ui)] flex justify-between items-center bg-[var(--bg-card)]">
                <Heading sub="Project Info">{editingId ? 'Edit Mission' : 'New Mission'}</Heading>
                <button onClick={closeForm} className="p-2 text-slate-400 hover:text-rose-500 transition-all"><X size={24} /></button>
             </header>
                ...
                {step === 0 ? (
                  <div className="space-y-10 py-10 text-center max-w-xl mx-auto">
                     <div className="space-y-4">
                        ...
                        <h4 className="text-2xl font-bold tracking-tight">AI Mission Planner</h4>
                        <p className="text-xs font-medium text-slate-500">Describe your strategic objective below.</p>
                     </div>
                     <textarea 
                      ...
                      placeholder="e.g. Build a luxury real estate portal for high-net-worth clients..." 
                      value={aiPrompt} 
                      onChange={e => setAiPrompt(e.target.value)} 
                     />
                     <div className="space-y-6">
                        <CustomSelect label="Choose Client Node" value={formData.clientId} options={clients.map(c => ({ id: c.id, label: c.name }))} icon={Building2} onChange={(id: string) => { const c = clients.find(cl => cl.id === id); setFormData({...formData, clientId: id, clientName: c?.name || ''}); }} />
                        <div className="flex gap-3">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                    <div className="lg:col-span-8 space-y-8">
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <Input label="Mission Name" placeholder="e.g. Web Infrastructure Design" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
                          <CustomSelect label="Client Node" value={formData.clientId} options={clients.map(c => ({ id: c.id, label: c.name }))} icon={Building2} onChange={(id: string) => { const c = clients.find(cl => cl.id === id); setFormData({...formData, clientId: id, clientName: c?.name || ''}); }} />
                       </div>
                       <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                          <TemporalPicker label="Start Date" value={formData.startDate || ''} onChange={(v) => setFormData({...formData, startDate: v})} />
                          <TemporalPicker label="Target Date" value={formData.timeline || ''} onChange={(v) => setFormData({...formData, timeline: v})} />
                          <Input label="Sector" placeholder="e.g. Real Estate" value={formData.industry} onChange={e => setFormData({...formData, industry: e.target.value})} />
                       </div>
                       <div className="space-y-2">
                          <Label>Objective Description</Label>
                          <textarea className="w-full bg-[var(--input-bg)] border border-[var(--border-ui)] rounded-xl p-4 text-sm font-medium outline-none focus:border-[var(--accent)] transition-all min-h-[160px] leading-relaxed" placeholder="Define mission parameters..." value={formData.scope} onChange={e => setFormData({...formData, scope: e.target.value})} />
                       </div>
                    </div>
                    <div className="lg:col-span-4 space-y-8">
                       <Card variant="muted" className="space-y-6">
                          <Heading sub="Financials">Mission Worth</Heading>
                          ...
                          <Select label="Billing Modality" value={formData.billingType} onChange={e => setFormData({...formData, billingType: e.target.value as any})}>
                       ...
                       <div className="pt-4 space-y-3">
                          <Button onClick={finalizeSave} className="w-full h-12" disabled={!formData.clientId || !formData.title}>Register Mission</Button>
      ...
      <ConfirmationModal isOpen={!!confirmDeleteId} title="Decommission Node" message="Are you sure you wish to remove this project mission from the registry?" onConfirm={() => { if (confirmDeleteId) deleteProposal(confirmDeleteId); setConfirmDeleteId(null); }} onCancel={() => setConfirmDeleteId(null)} />
    </div>
  );
};
